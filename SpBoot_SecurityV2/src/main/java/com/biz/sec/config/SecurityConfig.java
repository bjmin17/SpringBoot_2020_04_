...
//		super.configure(web);
	 ...
	 * 
	 * 	react, angular 같은 별도의 view를 사용하여 연동할 때는
	 * 	csrf를 처리할 수 있는 방법이 거의 없다.
	 * 
	 *  따라서 react 등을 사용할 때는
	 *  csrf token을 검증하는 절차를 생략하고 사용할 수밖에 없다.
	 ...
	 * 
	 *  이런 경우 react 등에서는 서버에서 어떤 반응이 있는지를
	 *  전혀 인지할 수 없다.
	 ...
	 * 	jsp의 sec:, authority:, thymeleaf의 sec: 같은 lib tag에서
	 * 	로그인한 사용자 정보를 조회하는 코드가 있는데
	 * 	이 코드들은 실제 security와 연동된 httpSession을 사용한다.
	 *  
	 *  이 기능 또한 react등을 연동할때는 사용하지 않도록 정책설정을 한다.
	 *  
	 *  spring과 react 등이 연동할 때는 jwt라는 보안 기능을 사용한다.
	 ...
	 *  사용자의 로그인 상태, ROLE 상태 등을 인지할 수 있는 방법이 없다.
	 *  
	 *  react와 기본 template을 연동할때는
	 *  session 기능을 열어두고
...
//		super.configure(http);
		...
		// login 페이지는 user/login 페이지로 열겠다.
		httpSecurity.authorizeRequests()
			.and()
			.formLogin()
